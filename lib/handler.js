...
module.exports = class PdfHandler {
	...
	static iccdir = Path.join(__dirname, 'icc');
	...
	constructor(opts) {
		this.#settings = Object.assign({}, PdfHandler, opts);
		this.chain = (...args) => this.helper(...args);
	}

	helper({ location, plugins, online }, req, res) {
		const { presets, timeout, policies } = this.#settings;
		const pdf = location.searchParams.get('pdf');
	...
	plugin(page, settings, req, res) {
		...
		page.on('idle', async () => {
			...
			stream.pipe(res);
			once(stream, 'finish').then(() => unlink(outputPath));
		});
	}
