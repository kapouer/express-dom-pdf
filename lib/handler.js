...
const autobreakClient = await readFile('./autobreak');
...
module.exports = class PdfHandler {
	...
	plugin(page, settings, req, res) {
		const { presets, autobreak } = this.#settings;
		...
		page.on('idle', async () => {
			const title = getSlug(await page.title() || Path.basename(
			...
			if (autobreak) await page.evaluate(autobreakClient, autobreak);
